...
import { Radio, Ambulance, LayoutDashboard, Building2, User } from 'lucide-react';
  ...
  const getMobileLinks = () => {
    ...
    if (role === 'dispatcher') {
      return [
        ...
        { label: 'Hospitals', path: '/dispatcher/hospitals', icon: <Building2 className="w-5 h-5" /> },
      ];
    }
    if (role === 'hospital_admin') {
      return [
        { label: 'Trauma', path: '/hospital', icon: <Building2 className="w-5 h-5" /> },
        { label: 'Inbound', path: '/hospital/emergency', icon: <Radio className="w-5 h-5" /> },
        { label: 'Beds', path: '/hospital/resources', icon: <User className="w-5 h-5" /> },
      ];
    }
    return [
      { label: 'SOS Home', path: '/citizen', icon: <Radio className="w-5 h-5" /> },
      { label: 'Report', path: '/citizen/report', icon: <Radio className="w-5 h-5" /> },
      { label: 'Tracking', path: '/citizen/tracking', icon: <Ambulance className="w-5 h-5" /> },
      { label: 'Profile', path: '/citizen/profile', icon: <User className="w-5 h-5" /> },
    ];
  };
